...
def process_liq_iibb_osebal(anio, mes):
    ...
    try:
        try:
            ARCH_ZIP = zipfile.ZipFile(settings.MEDIA_ROOT + r'/liq_iibb/' + 'LiqIIBB_OSeBal_'
                                       + str(anio) + str(mes).zfill(2) + '.zip',
                                       mode='x', compression=zipfile.ZIP_DEFLATED)
        except FileExistsError:
            os.remove(settings.MEDIA_ROOT + r'/liq_iibb/' + 'LiqIIBB_OSeBal_'
                      + str(anio) + str(mes).zfill(2) + '.zip')
            ARCH_ZIP = zipfile.ZipFile(settings.MEDIA_ROOT + r'/liq_iibb/' + 'LiqIIBB_OSeBal_'
                                       + str(anio) + str(mes).zfill(2) + '.zip',
                                       mode='x', compression=zipfile.ZIP_DEFLATED)
    ...
    if log:
        with io.open(settings.MEDIA_ROOT + r'/liq_iibb/' + 'Log_OSeBal_' + str(anio)
                     + '-' + str(mes).zfill(2) + '.txt', 'wt', newline='\r\n') as logarch:
            logarch.write('\n'.join(log))
        return (settings.MEDIA_ROOT + r'/liq_iibb/' + 'Log_OSeBal_' + str(anio)
                + '-' + str(mes).zfill(2) + '.txt')
    else:
        return (settings.MEDIA_ROOT + r'/liq_iibb/' + 'LiqIIBB_OSeBal_'
                + str(anio) + str(mes).zfill(2) + '.zip')
